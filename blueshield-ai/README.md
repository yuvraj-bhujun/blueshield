...
Then open **http://localhost:5000** — it redirects straight to `/coastguard`.

## Pages

This build only ships two pages:

- `/coastguard` — maritime intelligence dashboard + AI investigation assistant
- `/wakashio` — the Wakashio replay demo scenario

All backend APIs (vessels, alert-status/"signal", speech, geofencing, reef data,
etc.) remain unchanged and fully intact.
...
- `templates/` — `base.html` layout plus one template per remaining page:
...
- `static/js/main.js` — shared nav behaviour
- `static/js/coastguard.js`, `static/js/wakashio.js` — per-page map rendering, polling, and AI-panel wiring
